...
from utils import AverageMeter, compute_metrics, MaxIterSampler, MultiSourceDataset
...
from Multimodal import model_entry
...
def load_data(configs):
    ...
    dataset = MultiSourceDataset(configs.csv_dir,configs.hsi_dir,configs.lidar_dir,configs.r,transform)
    # dataset = MSDataset(configs.csv_dir,configs.hsi_dir,configs.lidar_dir,2*configs.r+1,transform)
...
def data_preprocessing(data):
    hsi = data['hsi'].float().cuda()
    lidar = data['lidar'].float().cuda()
    ...
    y = data['y']
    x = data['x']
    return hsi, lidar, labels, batch_size, y, x
...
def train_model(log, log_dir, configs):
    ...
    # torch.save(best_model.cpu().state_dict(), os.path.join(log_save_dir, 'initial' + "_" + str(seed) + ".pth"))
    ...
    for i, data in enumerate(dataloader_train):
        ...
        hsi, lidar, labels, batch_size, _, _ = data_preprocessing(data)
        optimizer.zero_grad()
        out = model.forward(hsi, lidar)
        loss = model.compute_loss(out, hsi, lidar, labels)
        loss.backward()
        optimizer.step()

        Pre_label = model.compute_pre_label(out)
        pre_true_num = torch.sum(torch.eq(Pre_label, labels)).item()
        acc = pre_true_num / batch_size # int64Tensor/int=0, pre_true_num should also be int type
        ...
        if current_iter != 0 and current_iter % configs.valid_iter == 0:
            ...
            with torch.no_grad():
                for batch_idx, data in enumerate(dataloader_valid):
                    hsi, lidar, labels, batch_size, y, x = data_preprocessing(data)
                    out = model.forward(hsi, lidar)
                    loss = model.compute_loss(out, hsi, lidar, labels)
